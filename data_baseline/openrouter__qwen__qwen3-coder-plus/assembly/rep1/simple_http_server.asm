...
; Constants structure (in BSS for alignment)
struc sockaddr_in
    .sin_family: resw 1
    .sin_port:   resw 1  
    .sin_addr:   resd 1
...
.done:
    ; Convert to network byte order (flip high/low byte)
    rol ax, 8
    mov rbx, rax
    rol ax, 8
    and eax, 0xFFFF0000  ; clear low bits
    or eax, ebx          ; set port in low bits
    mov rbx, rax
    rol rbx, 16
    
    mov [port_number], bx ; store final value
    pop rdx
    pop rcx  
    pop rbx
    pop rbp
...
start_server:
    ...
    mov word [addr_struct + sockaddr_in.sin_family], AF_INET
    mov word [addr_struct + sockaddr_in.sin_port], 0    ; will be stored below
    mov dword [addr_struct + sockaddr_in.sin_addr], 0   ; INADDR_ANY = 0.0.0.0
    
    ; Get actual port and store it in network byte order  
    mov ax, [port_number]
    mov [addr_struct + sockaddr_in.sin_port], ax
...
handle_request:
    ...
    mov rsi, r12      ; request buffer start
    mov rdi, temp_method
    call parse_word_until_space  ; Extract method like "GET", "POST", etc.
    
    ; Now parse resource path after method and space
    add rsi, 1        ; rsi += length of method + 1 space
.seek_to_path:
    mov al, [r12 + rsi]
    cmp al, ' '
    je .found_path_start
    inc rsi
    cmp rsi, r14      ; prevent accessing beyond received bytes
    jb .seek_to_path
    ; If didn't find space, something is wrong
    
.found_path_start:
    inc rsi           ; skip the space
    mov rdi, temp_path
    call parse_word_until_space  ; Extract path like "/login", "/todos", etc.
...
parse_word_until_space:
    ...
    mov rsi, temp_request_cursor  ; start from where we left off
    mov rdi, temp_output_cursor   ; output location
...
.parse_loop:  
    ...
    mov [rdi + rcx], al
    inc rcx
    cmp rcx, 100     ; max word length
    jb .parse_loop
    
.parse_done:
    mov byte [rdi + rcx], 0     ; null terminate
    mov eax, rcx                ; return length
    ; Update cursor position globally
    add [temp_request_cursor], rax
    inc qword [temp_request_cursor]  ; add 1 more for space
...
.handle_get_todo_by_id:
    ...
    mov rdi, msg_404_not_found  ; For now, since we track no todos
    mov rsi, len_404_not_found
    mov rdx, r13
    call send_response
    jmp .done
...
check_authenticated:
    ...
    ; For now, allow all if it looks like auth headers exist
    ; Look for "Cookie:" or "Authorization:" headers
...
section .bss
    ...
    server_fd resd 1
    port_number resw 1
    request_cursor resq 1      ; tracks current position in request
    temp_request_cursor resq 1
    temp_output_cursor resq 1
    
    ; Fixed-size strings (parsing)
    temp_method resb 16        ; max length for HTTP method
    temp_path resb 128         ; for URL path  
    temp_request_headers resb 2048  ; store request headers
    ...
    ; Socket structure  
    addr_struct:
        resw sockaddr_in.sin_family
        resw sockaddr_in.sin_port
        resd sockaddr_in.sin_addr  
        resd sockaddr_in.padding   ; for alignment
    sockaddr_in_size equ $ - addr_struct  ; Should be 16 bytes
...
.found_method_end:
    ...
    ; Now start at position after method and space to parse path
    lea rdi, [.temp_path]      ; Where to store parsed path
    lea r11, [rbx + rsi + 1]   ; r11 = offset pointing just after method + space
    mov rcx, 0                 ; Current position in path

.copy_path_chars:
    mov al, [r11 + rcx]        ; Character from request at position r11+rcx

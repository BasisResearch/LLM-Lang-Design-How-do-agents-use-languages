...
start_server:
    ...
    mov word [addr_struct + 2], [port_number]          ; sin_port 
    mov dword [addr_struct + 4], 0                     ; sin_addr - INADDR_ANY = 0.0.0.0
    mov dword [addr_struct + 8], 0                     ; padding
    mov dword [addr_struct + 12], 0                    ; padding
...
handle_request:
    ...
    mov r8, 0         ; start at beginning
    mov r9, 0         ; count characters for method
    
.scan_method:
    mov al, [r12 + r8]
    cmp al, ' '
    je .found_method_delim
    mov [temp_method + r9], al
    inc r8
    inc r9
    cmp r9, 15        ; max method length
    jl .scan_method
    ; Too long method, just continue
    
.found_method_delim:
    mov byte [temp_method + r9], 0   ; null terminate
    inc r8                           ; skip the space
    
    ; Now parse resource path
    mov r9, 0                        ; count characters for path
...
.scan_path:
    ...
    mov [temp_path + r9], al
    inc r9
    cmp r9, 127       ; max path length
    jl .scan_path

.found_path_delim:
    mov byte [temp_path + r9], 0   ; null terminate
...
perform_registration:
    ; Extract username and password from request body (placeholder)
    mov rax, 1   ; Return 1 for success
    ret

validate_login:
    ; Extract username and password from body and validate (placeholder)  
    mov rax, 1   ; Success
    ret

check_authenticated:
    ; Would check cookies/sessions here (placeholder)

...
/* Simplified and corrected session lookup function */
int find_session_by_id(const char* session_id) {
    printf("DEBUG: Looking for session ID: '%s'\n", session_id);
    printf("DEBUG: Number of sessions: %d\n", num_sessions);
    
    for (int i = 0; i < num_sessions; i++) {
        if (sessions[i].active) {
            printf("DEBUG: Checking session[%d]: '%s' (user_id: %d)\n", i, sessions[i].id, sessions[i].user_id);
            if (strcmp(sessions[i].id, session_id) == 0) {
                printf("DEBUG: Found matching session at index %d\n", i);
                return i;
            }
        }
    }
    printf("DEBUG: No matching session found\n");
    return -1;
}

char* extract_cookie_value(const char* headers, const char* cookie_name) {
    printf("DEBUG: Extracting cookie '%s'\n", cookie_name);
    
    // Looking for "Cookie:" header
    const char* cookie_header = strstr(headers, "Cookie:");
    if (!cookie_header) {
        printf("DEBUG: No 'Cookie:' header found\n");
        return NULL;
    }
    
    // Point to after "Cookie: "
    cookie_header += 7;
    printf("DEBUG: Cookie data: '%s'\n", cookie_header);
    
    // Find session_id using string manipulation instead of regex to avoid complexity
    char pattern[100];
    snprintf(pattern, sizeof(pattern), "%s=", cookie_name);
    
    const char* pos = strstr(cookie_header, pattern);
    if (!pos) {
        printf("DEBUG: Pattern '%s' not found in cookies\n", pattern);
        return NULL;
    }
    
    // Point to the value after =
    pos += strlen(pattern);
    
    // Find the end of the value (next semicolon or end of string)
    const char* end = strchr(pos, ';');
    if (!end) {
        end = strchr(pos, ',');  // Alternative separator sometimes used
        if (!end) {
            end = pos + strlen(pos);  // End of string
        }
    }
    ...
    printf("DEBUG: Extracted cookie value: '%s'\n", value);
    return value;
}
...
int extract_json_string_field(const char* json, const char* field_name, char* output, int output_size) {
    ...
    while (*pos) {
        if (!escaped && *pos == '\\') {
            ...
            copied++;
            escaped = escaped ? 0 : 0;  // Reset escape flag for non-escape chars
        }
        pos++;
    }
...
void handle_login(int client_socket, const char* body) {
    ...
    printf("DEBUG: Created new session with ID: '%s' for user ID: %d at index %d\n", session->id, session->user_id, num_sessions);
...
int get_current_user_id_from_header(const char* headers) {
    printf("DEBUG: Getting user ID from headers\n");
    char* session_id = extract_cookie_value(headers, "session_id");
    if (!session_id) {
        printf("DEBUG: Could not extract session_id from headers\n");
        return -1;
    }
    
    int session_idx = find_session_by_id(session_id);
    if (session_idx == -1 || !sessions[session_idx].active) {
        printf("DEBUG: Session not found or inactive for ID: %s\n", session_id);
        return -1;
    }
    
    printf("DEBUG: Returning user ID: %d for session ID: %s\n", sessions[session_idx].user_id, session_id);
...
void handle_request(int client_socket, const char* method, const char* url, const char* headers, const char* body) {
    ...
    if (auth_required) {
        ...
        if (user_id == -1) {
            printf("DEBUG: Authentication failed for %s\n", url);
            send_response(client_socket, 401, "{\"error\": \"Authentication required\"}", 1);
            return;
        }
        printf("DEBUG: Authentication succeeded for user ID: %d\n", user_id);
...
int main(int argc, char *argv[]) {
    ...
    while(1) {
        ...
        printf("DEBUG: Processing request: %s %s\n", method, url);

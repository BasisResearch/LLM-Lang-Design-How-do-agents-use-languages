...
char* extract_json_field(const char *body, const char* field_name) {
    ...
    char *field_pos = strstr((char*)body, search_pattern);
    if (!field_pos) {
        return NULL;
    }
    ...
    if (*field_pos == '"') {
        ...
        if (end_quote) {
            int length = end_quote - field_pos;
            char *result = malloc(length + 1);
            strncpy(result, field_pos, length);
            result[length] = '\0';
            return result;
        }
    } else {
        // Non-string value (number, boolean)
        char *end_field = field_pos;
        while (*end_field != '}' && *end_field != ',' && *end_field != ']' && 
               *end_field != ' ' && *end_field != '\t' && *end_field != '\n' && *end_field != '\r') {
            end_field++;
        }
        int length = end_field - field_pos;
        char *result = malloc(length + 1);
        strncpy(result, field_pos, length);
        result[length] = '\0';
        return result;
...
void handle_update_todo(int client_fd, const char* headers, const char* request_body, int todo_id) {
    ...
    if (completed_str) {
        // Compare with lowercase versions to handle different formats of booleans
        if (strlen(completed_str) == 4 && strncasecmp(completed_str, "true", 4) == 0) {
            todo->completed = 1;
        } else if (strlen(completed_str) == 5 && strncasecmp(completed_str, "false", 5) == 0) {
            todo->completed = 0;
        } else if (strcmp(completed_str, "1") == 0) {
            todo->completed = 1;
        } else if (strcmp(completed_str, "0") == 0) {
            todo->completed = 0;
        }
    }
...
void* handle_request_thread(void* arg) {
    ...
    // Read all of the request into a buffer
    char request[8192];
    int offset = 0;
    int bytes_read;
    ...
    while(offset < 8191) {
        bytes_read = recv(client_fd, request + offset, 1, 0);
    ...
    // Now read the rest of the specified body content-length
    request[offset] = '\0';  // Null terminate
    
    char *content_length_header = strstr(request, "Content-Length:");
    if(content_length_header) {
        content_length_header += 15;   // Length of "Content-Length:"
        ...
        if(body_start) {
            body_start += 4;
            int already_filled = request + offset - body_start;
            
            if(content_length > already_filled) {
                int remaining = content_length - already_filled;
                
                // Now read the remaining body bytes
                int body_bytes_read = 0;
                while(body_bytes_read < remaining && offset < 8191) {
                    int next_read = recv(client_fd, request + offset, 1, 0);
                    if(next_read <= 0) break;
                    
                    offset += next_read;
                    body_bytes_read += next_read;
                }
            }
        }
    }
    
    request[offset] = '\0';  // Ensure termination
    ...
    else if (strcasecmp(method, "POST") == 0 && strcmp(path, "/login") == 0) {
        handle_login(client_fd, body);
    } 
    else if (strcasecmp(method, "POST") == 0 && strcmp(path, "/logout") == 0) {
        handle_logout(client_fd, request);
    } 
    else if (strcasecmp(method, "GET") == 0 && strcmp(path, "/me") == 0) {
        handle_get_me(client_fd, request);
    } 
    else if (strcasecmp(method, "PUT") == 0 && strcmp(path, "/password") == 0) {
        handle_change_password(client_fd, request, body);
    } 
    else if (strcasecmp(method, "GET") == 0 && strcmp(path, "/todos") == 0) {
        handle_get_todos(client_fd, request);
    } 
    else if (strcasecmp(method, "POST") == 0 && strcmp(path, "/todos") == 0) {
        handle_create_todo(client_fd, request, body);
    } 
    else if (strcmp(path, "/password") == 0) {
        // Additional check for PUT method
    ...
    else if (strstr(path, "/todos/") == path) {
        // Extract todo ID from path
        int todo_id;
        if (sscanf(path, "/todos/%d", &todo_id) == 1) {
            ...
            send_error(client_fd, 400, "Invalid todo ID");
        }
    }
    else {
        send_error(client_fd, 404, "Not Found");

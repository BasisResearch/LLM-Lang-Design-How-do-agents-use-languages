...
def test_server_endpoints():
    ...
    try:
        ...
        session.post('http://localhost:8080/logout')
        
        # Test with wrong old password
        session2 = requests.Session()
        response = session2.post(
            'http://localhost:8080/login',
            json=test_user  # Still contains old password
        )
        assert response.status_code == 401  # Should fail since password was changed
        ...
        response = session2.get('http://localhost:8080/todos')
        assert response.status_code == 200
        ...
        response = session2.get(f"http://localhost:8080/todos/{todo1['id']}")
        assert response.status_code == 200
        ...
        response = session2.get("http://localhost:8080/todos/99999")
        ...
        response = session2.put(f"http://localhost:8080/todos/{todo1['id']}", json=update_data)
        assert response.status_code == 200
        ...
        response = session2.put(f"http://localhost:8080/todos/{todo2['id']}", json=partial_update)
        assert response.status_code == 200
        ...
        bad_update = {"title": ""}
        response = session2.put(f"http://localhost:8080/todos/{todo2['id']}", json=bad_update)
        assert response.status_code == 400
        assert response.json()['error'] == 'Title is required'
        ...
        response = session2.delete(f"http://localhost:8080/todos/{todo1['id']}")
        ...
        # Test logout
        response = session2.post('http://localhost:8080/logout')
        assert response.status_code == 200
        
        # After logout, session should be invalid
        response = session2.get('http://localhost:8080/me')
        assert response.status_code == 401

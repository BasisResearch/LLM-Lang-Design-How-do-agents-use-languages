...
object TestHttp4s extends IOApp.Simple {
  ...
  def routes: HttpRoutes[IO] = HttpRoutes.of[IO] {
    ...
    case GET -> Root / "b" => OptionT.liftF(Response[IO](Status.Unauthorized).withEntity(Err("test2")))
    case GET -> Root / "c" => OptionT.liftF(Response[IO](Status.NotFound).withEntity(Err("test3")))
    case GET -> Root / "d" => OptionT.liftF(Response[IO](Status.Conflict).withEntity(Err("test4")))
  }

...
echo "DEBUG Update Response Body: $BODY_res"
if [ "$CODE" != "200" ]; then
...
curl -s -X POST "$BASE_URL/register" -H "Content-Type: application/json" -d '{"username":"otheruser","password":"password123"}' -c cookies3.txt > /dev/null
